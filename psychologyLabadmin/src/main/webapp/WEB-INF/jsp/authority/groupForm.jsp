...
<body class="">
	...
	<div class="content">
		...
		<ul class="breadcrumb">
			...
			<li class="active">
			<c:choose>
				<c:when test="${group.groupId > 0 && page>0 }">
				修改
				</c:when>
				<c:otherwise>
				新增
			...
			<div class="row-fluid">
				...
				<div class="btn-toolbar">
					...
					<c:if test="${group.groupId > 0}">
						...
						<a href="<c:url value="/groups/${page}"/>" data-toggle="modal" class="btn">返回角色列表</a>
					</c:if>
				...
				<div class="well">
					<div id="myTabContent" class="tab-content">
						<div class="tab-pane active in" id="home">
							<form:form modelAttribute="group" id="groupForm" method="post">
								<label>名称</label> 
								<input type="text" id="name" name="name" value="${group.name}" class="input-xlarge"> 
								<span id="nameError" class="text-error"></span>
								<label>描述</label> 
								<input type="text" name="description" value="${group.description }" class="input-xlarge"/> 
								<label>状态</label> 
								<select name="enable" class="input-xlarge">
								...
								<c:forEach items="${allRoles }" var="role" varStatus="status">
									...
									<form:checkbox path="roles" value="${role.roleId },${role.name }"/>${role.name }
									</label>
